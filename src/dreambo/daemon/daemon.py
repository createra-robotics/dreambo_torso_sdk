...
from dreambo.daemon.robot_app_lock import RobotAppLock
from dreambo.daemon.utils import (
    SimulationMode,
    find_serial_port,
    get_ip_address,
)
from dreambo.io.protocol import DaemonState, DaemonStatus, MotorControlMode
from dreambo.io.ws_server import WSServer
from dreambo.tools.reflash_motors import reflash_motors_if_needed
...
class Daemon:
    ...
    def __init__(
        ...
        # Get package version
        try:
            package_version = version("dreambo")
    ...
    async def _start_central_signaling_relay(self) -> None:
        ...
        try:
            from dreambo.media.central_signaling_relay import start_central_relay
    ...
    async def _stop_central_signaling_relay(self) -> None:
        ...
        try:
            from dreambo.media.central_signaling_relay import stop_central_relay
    ...
    async def start(
        ...
        self.logger.info("Starting Dreambo daemon...")
    ...
    async def stop(self, goto_sleep_on_stop: bool = True) -> "DaemonState":
        ...
        Args:
            goto_sleep_on_stop (bool): If True, put Dreambo to sleep on stop. Defaults to True.
        ...
        try:
            ...
            self.logger.info("Stopping Dreambo daemon...")
            self._status.state = DaemonState.STOPPING
            ...
            if goto_sleep_on_stop:
                try:
                    self.logger.info("Putting Dreambo to sleep...")
                    self.backend.set_motor_control_mode(MotorControlMode.Enabled)
                    await self.backend.goto_sleep()
                    self.backend.set_motor_control_mode(MotorControlMode.Disabled)
                except Exception as e:
                    self.logger.error(f"Error while putting Dreambo to sleep: {e}")
                    self._status.state = DaemonState.ERROR
                    self._status.error = str(e)
                except KeyboardInterrupt:
    ...
    async def restart(
        ...
        Args:
            ...
            wake_up_on_start (bool): If True, wake up Dreambo on start. Defaults to None (don't wake up).
            goto_sleep_on_stop (bool): If True, put Dreambo to sleep on stop. Defaults to None (don't go to sleep).
        ...
        if self._status.state in (DaemonState.RUNNING, DaemonState.ERROR):
            self.logger.info("Restarting Dreambo daemon...")
    ...
    def status(self) -> "DaemonStatus":
        """Get the current status of the Dreambo daemon."""
        if self.backend is not None:
    ...
    def _setup_backend(
        ...
        if mockup_sim:
            ...
            if serialport == "auto":
                ...
                if len(ports) == 0:
                    raise RuntimeError(
                        "No Dreambo serial port found. "
                        "Check USB connection and permissions. "
                        "Or directly specify the serial port using --serialport."
                    )
                elif len(ports) > 1:
                    raise RuntimeError(
                        f"Multiple Dreambo serial ports found {ports}. "
