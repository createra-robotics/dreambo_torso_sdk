...
from dreambo.io.publisher import Publisher

if typing.TYPE_CHECKING:
    from dreambo.kinematics import AnyKinematics
# MediaManager no longer used here — play_sound delegated to GstMediaServer
from dreambo.media.audio_doa import AudioDoA
from dreambo.motion.goto import GotoMove
from dreambo.motion.move import Move
from dreambo.utils.constants import MODELS_ROOT_PATH, URDF_ROOT_PATH
from dreambo.utils.interpolation import (
...
class Backend:
    ...
    def __init__(
        ...
        if self.kinematics_engine == "Placo":
            ...
            from dreambo.kinematics import NNKinematics
    ...
    def process_command(
        ...
        if isinstance(cmd, SetTargetCmd):
            ...
            send_response({"version": version("dreambo")})
            ...
            from dreambo.daemon.app.routers.volume_control import (
                get_volume_control,
            )
