"""Robot Backend for Dreambo.

This module provides the `RobotBackend` class, which interfaces with the Dreambo motor controller to control the robot's movements and manage its status.
...
from dreambo.io.protocol import (
    HeadPoseMsg,
    ImuDataMsg,
    JointPositionsMsg,
    MotorControlMode,
...
class RobotBackend(Backend):
    """Real robot backend for Dreambo."""
    ...
    def __init__(
        ...
        Args:
            serialport (str): The serial port to which the Dreambo is connected.
            ...
            wireless_version (bool): If True, indicates that the wireless version of Dreambo is used. Default is False.
            hardware_config_filepath (str | None): Path to the hardware configuration YAML file. Default is None.

        Tries to connect to the Dreambo motor controller and initializes the control loop.
