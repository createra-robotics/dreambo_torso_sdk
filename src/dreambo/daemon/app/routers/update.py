"""Update router for Dreambo Daemon API.
...
from dreambo.daemon.app import bg_job_register
from dreambo.daemon.app.bg_job_register import JobInfo
from dreambo.utils.wireless_version.update import update_dreambo
from dreambo.utils.wireless_version.update_available import (
    get_install_source,
    get_local_version,
    get_pypi_version,
    is_update_available,
...
def install_source() -> dict[str, str]:
    ...
    return get_install_source("dreambo")
...
def available(pre_release: bool = False) -> dict[str, dict[str, dict[str, bool | str]]]:
    ...
    current_version = str(get_local_version("dreambo"))

    try:
        is_available = is_update_available("dreambo", pre_release)
        available = str(get_pypi_version("dreambo", pre_release))
...
def start_update(pre_release: bool = False) -> dict[str, str]:
    """Start the update process for Dreambo Wireless version."""
    if busy_lock.locked():
        raise HTTPException(status_code=400, detail="Update already in progress")

    if not is_update_available("dreambo", pre_release):
        raise HTTPException(status_code=400, detail="No update available")

    async def update_wrapper(logger: logging.Logger) -> None:
        with busy_lock:
            await update_dreambo(logger, pre_release=pre_release)
...
GITHUB_REPO = "createra-robotics/dreambo_torso_sdk"
...
def start_update_from_ref(git_ref: str) -> dict[str, str]:
    ...
    async def update_wrapper(logger: logging.Logger) -> None:
        with busy_lock:
            await update_dreambo(logger, git_ref=git_ref)

    job_uuid = bg_job_register.run_command("update_dreambo", update_wrapper)
    return {"job_id": job_uuid}
