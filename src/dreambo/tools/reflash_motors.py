"""Script to reflash Dreambo Robot servos firmware."""
...
import dreambo
from dreambo.daemon.utils import find_serial_port
from dreambo.tools.setup_motor import (
    check_configuration,
    light_led_down,
    light_led_up,
    setup_motor,
)
from dreambo.utils.hardware_config.parser import parse_yaml_config
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Reconfigure Dreambo servos' registers.")
    parser.add_argument(
        "--serialport",
        type=str,
        required=False,
        default=None,
        help="Serial port of the Dreambo robot torso(e.g. /dev/ttyUSB0 or COM3). "
...
def reflash_motors_if_needed(
    ...
    """Reflash Dreambo's motors."""
    console = Console()

    config_file_path = str(
        files(dreambo).joinpath("assets/config/hardware_config.yaml")
    )
    config = parse_yaml_config(config_file_path)
    motors = list(config.motors.keys())
    if serialport is None:
        console.print(
            "Which version of Dreambo are you using?",
        )
        ...
        serialport = ports[0]
        console.print(f"Found Dreambo serial port: {serialport}", style="green")
