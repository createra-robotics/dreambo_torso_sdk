"""Dreambo Torso Application Base Class.

This module provides a base class for creating Dreambo applications.
...
class ReachyMiniApp(ABC):
    """Base class for Dreambo applications."""
    ...
    def __init__(self, running_on_wireless: bool = False) -> None:
        ...
        # Media backend is now auto-detected by Dreambo, just use "default"
        self.media_backend = (
    ...
    def wrapped_run(self, *args: Any, **kwargs: Any) -> None:
        """Wrap the run method with Dreambo context management."""
        ...
        try:
            self.logger.info("Starting Dreambo app...")
    ...
    @abstractmethod
    def run(self, reachy_mini: Dreambo, stop_event: threading.Event) -> None:
        """Run the main logic of the app.

        Args:
            reachy_mini (Dreambo): The Dreambo instance to interact with.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="App creation and publishing assistant for Dreambo."
    )
    ...
    publish_parser = subparsers.add_parser(
        "publish", help="Publish the app to the Dreambo app store"
    )
    ...
    publish_parser.add_argument(
        ...
        required=False,
        default=False,
        help="Request to publish the app as an official Dreambo app.",
    )
    publish_parser.add_argument(
        "--nocheck",
        action="store_true",
...
def main() -> None:
    ...
    if args.command == "create":
        if args.template == "conversation":
            from dreambo.apps.fork_conversation import create_from_conversation_app
