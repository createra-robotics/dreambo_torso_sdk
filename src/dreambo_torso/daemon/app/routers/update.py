...
from dreambo_torso.utils.wireless_version.update_available import (
    get_install_source,
    get_local_version,
    get_pypi_version,
    is_update_available,
...
def install_source() -> dict[str, str]:
    ...
    return get_install_source("dreambo_torso")
...
def available(pre_release: bool = False) -> dict[str, dict[str, dict[str, bool | str]]]:
    ...
    current_version = str(get_local_version("dreambo_torso"))

    try:
        is_available = is_update_available("dreambo_torso", pre_release)
        available = str(get_pypi_version("dreambo_torso", pre_release))
...
def start_update(pre_release: bool = False) -> dict[str, str]:
    ...
    if not is_update_available("dreambo_torso", pre_release):
