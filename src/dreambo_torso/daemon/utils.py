"""Utilities for managing the Dreambo Torso daemon."""
...
class SimulationMode(Enum):
    """Simulation mode for the Dreambo Torso daemon."""
...
CAMERA_SOCKET_PATH = "/tmp/dreambo_camera_socket"
...
CAMERA_PIPE_NAME = "\\\\.\\pipe\\dreambo_camera_pipe"
...
def daemon_check(spawn_daemon: bool, use_sim: bool) -> None:
    ...
    if spawn_daemon:
        daemon_is_running, pid, sim = is_python_script_running("dreambo-torso-daemon")
        if daemon_is_running and sim == use_sim:
            print(
                f"Dreambo Torso daemon is already running (PID: {pid}). "
                "No need to spawn a new one."
            )
            return
        elif daemon_is_running and sim != use_sim:
            print(
                f"Dreambo Torso daemon is already running (PID: {pid}) with a different configuration. "
            )
        ...
        subprocess.Popen(
            ["dreambo-torso-daemon", "--sim"] if use_sim else ["dreambo-torso-daemon"],
...
def find_serial_port(
    ...
    """Find the serial port for Dreambo Torso based on VID and PID or the Raspberry Pi UART for the wireless version.
