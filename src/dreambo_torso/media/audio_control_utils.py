...
def find(vid: int = 0x2886, pid: int = 0x001A) -> ReSpeaker | None:
    ...
    Example:
        ```python
        from dreambo_torso.media.audio_control_utils import find
...
def init_respeaker_usb() -> Optional[ReSpeaker]:
    ...
    Example:
        ```python
        from dreambo_torso.media.audio_control_utils import init_respeaker_usb
    ...
    try:
        ...
        if dev is None:
            logger.error("No Audio USB device found!")
            return None
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Dreambo Robot Audio Host Control Script"
    )
    parser.add_argument(
        "command",
